...
map<string, ProficiencyLevel> LEVELS{{"team_lead", TEAM_LEAD},
                                     {"senior", SENIOR},
                                     {"expert", EXPERT},
                                     {"junior", JUNIOR}};
...
public:
  ...
  double get_tax_amount(double salary) {
    return util::rounded(salary * (double)tax_percentage / 100.0, 0);
  }
...
public:
  ...
  double get_tax_amount(Database db) {
    return db.get_salary_config(level).get_tax_amount(
        raw_salary + util::rounded(get_bonus_amount(db), 0));
  }
...
public:
  Team(Dictionary data) {
    ...
    bonus_working_hours_max_variance =
        stod(data["bonus_working_hours_max_variance"]);
    member_ids =
        util::string_to_int_vector(input::split(data["member_ids"], '$'));
    sort(member_ids.begin(), member_ids.end());
    bonus_percentage = 0;
  }
  int get_id() { return id; }
  int get_team_head_id() { return team_head_id; }
  string get_head_member_name(Database db) {
    return db.get_employee(team_head_id).get_name();
  }
  ...
  double get_average_member_working_hours(Database db) {
    return (double)get_total_working_hours(db) /
           (double)get_number_of_members(db);
  }
...
void Database::report_total_hours_in_range(int l, int r) {
  ...
  int max_working_hours =
      *max_element(total_working_hours + l, total_working_hours + r + 1);
  int min_working_hours =
      *min_element(total_working_hours + l, total_working_hours + r + 1);
...
void Database::update_salary_config(vector<string> input) {
  ...
  if (input[1] != "-")
    conf->set_base_salary(stoi(input[1]));
  if (input[2] != "-")
    conf->set_salary_per_hour(stoi(input[2]));
  if (input[3] != "-")
    conf->set_salary_per_extra_hour(stoi(input[3]));
  if (input[4] != "-")
    conf->set_official_working_hours(stoi(input[4]));
  if (input[5] != "-")
    conf->set_tax_percentage(stoi(input[5]));
  cout << "OK" << endl;
}
...
void Database::report_employee_per_hour(int l, int r) {
  ...
  for (int start = l; start < r; start++) {
    ...
    cout << start << '-' << start + 1 << ": ";
    rounded_working_avg[cur_time] = util::rounded(
        double(count_busy_employees(cur_time)) / MONTH_DAY_COUNT, 1);
...
int Database::get_total_working_hours_of_employee(int id) {
  return get_employee(id).get_total_working_hours();
}
...
void Database::get_salary_configs(string file_prefix) {
  ...
  for (int i = 1; i < (int)configs_raw_info.size(); i++)
    add_config(SalaryConfig(
        input::make_map(configs_raw_info[0], configs_raw_info[i])));
}
...
void Database::get_employees_input(string file_prefix) {
  ...
  sort(employees_raw_info.begin() + 1, employees_raw_info.end(),
       util::first_member_cmp);
  for (int i = 1; i < (int)employees_raw_info.size(); i++)
    add_employee(Employee(
        input::make_map(employees_raw_info[0], employees_raw_info[i]), *this));
}
...
void Database::get_teams_input(string file_prefix) {
  ...
  sort(teams_raw_info.begin() + 1, teams_raw_info.end(),
       util::first_member_cmp);
...
double
SalaryConfig::calculate_raw_salary(vector<WorkingDateTime> working_date_times) {
  double total_time = 0;
  for (auto wdt : working_date_times)
    total_time += wdt.get_length();
  double normal_earning =
      min(total_time, double(official_working_hours)) * salary_per_hour;
  double extra_earning =
      max(0.0, total_time - official_working_hours) * salary_per_extra_hour;
...
void Employee::print_salary_report() {
  ...
  cout << "Total Earning: " << fixed << setprecision(0) << total_earning
       << endl;
  cout << "---" << endl;
}
...
void Employee::print_detailed_salary_report(Database &db) {
  ...
  cout << "Tax: " << fixed << setprecision(0)
       << util::rounded(get_tax_amount(db), 0) << endl;
  cout << "Total Earning: " << fixed << setprecision(0) << total_earning
       << endl;
}
...
double Employee::get_bonus_amount(Database db) {
  if (!has_team())
    return 0.0;
  return round(raw_salary *
               (double)db.get_team(team_id).get_bonus_percentage() / 100.0);
}
...
void Team::report_salary(Database db) {
  ...
  cout << "Average Member Working Hours: " << fixed << setprecision(1)
       << util::rounded(get_average_member_working_hours(db), 1) << endl;
  cout << "Bonus: " << get_bonus_percentage() << endl;
  cout << "---" << endl;
  for (int id : member_ids) {
    cout << "Member ID: " << id << endl;
    cout << "Total Earning: " << fixed << setprecision(0)
         << db.get_employee(id).get_total_earning() << endl;
    cout << "---" << endl;
  }
}
  ...
  while (getline(cin, new_line)) {
    ...
    try {
      ...
      if (words.front() == "add_working_hours")
        db.add_working_hours(stoi(words[1]), stoi(words[2]), stoi(words[3]),
                             stoi(words[4]));
