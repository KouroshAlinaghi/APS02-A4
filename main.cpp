...
public:
  ...
  double get_tax_amount(Database db) {
    return db.get_salary_config(level).get_tax_amount(raw_salary +
                                                      get_bonus_amount(db));
  }
...
void get_employees_input(string file_prefix, Database &db) {
  ...
  for (int i = 1; i < (int)employees_raw_info.size(); i++)
    db.add_employee(
        Employee(make_map(employees_raw_info[0], employees_raw_info[i]), db));
}
...
void get_salary_configs(string file_prefix, Database &db) {
  ...
  for (int i = 1; i < (int)configs_raw_info.size(); i++)
    db.add_config(
        SalaryConfig(make_map(configs_raw_info[0], configs_raw_info[i])));
}
...
void Database::report_employee_per_hour(int l, int r) {
  ...
  for (int start = l; start < r; start++) {
    ...
    cout << start << '-' << start + 1 << ": ";
    rounded_working_avg[cur_time] =
        rounded(double(count_busy_employees(cur_time)) / MONTH_DAY_COUNT, 1);
...
SalaryConfig::calculate_raw_salary(vector<WorkingDateTime> working_date_times) {
  ...
  if (total_working_hours > official_working_hours)
    salary +=
        (double)((double)total_working_hours - (double)official_working_hours) *
        (double)salary_per_extra_hour;
...
void Employee::print_detailed_salary_report(Database &db) {
  ...
  cout << "Salary: " << fixed << setprecision(0) << get_raw_salary()
       << endl; // gerd kon agha
...
void Team::report_salary(Database db) {
  ...
  cout << "Average Member Working Hour: " << fixed << setprecision(1)
       << get_average_member_working_hours(db) << endl;
...
bool Team::is_eligible_for_bonus(
    Database &db) { // TODO:test this because testdata is dogshit
