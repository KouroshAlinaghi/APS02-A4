...
public:
  int calculate(vector<WorkingDateTime> working_date_times) {
    ...
    if (total_working_hours > official_working_hours)
      salary += (total_working_hours - official_working_hours) *
                salary_per_extra_hour;

    return salary;
  }
  int apply_tax(int salary) { return salary - get_tax_amount(salary); }
  int get_tax_amount(int salary) { return salary * tax_percentage / 100; }
  ...
  void update_configs(ProficiencyLevel level, int base_salary,
                      int salary_per_hour, int salary_per_extra_hour,
...
public:
  ...
  void add_to_team(Team team) { team_id = team.get_id(); }
  ...
  int get_tax_amount(Database db) {
    return db.get_salary_config(level).get_tax_amount(salary);
  }
...
public:
  ...
  int get_average_member_working_hours() {
    return get_total_working_hours() / get_number_of_members();
  }
